...
@if($pozadina)
    <img src="{{ $pozadina['url'] }}" alt="{{ $pozadina['alt'] ?? $naslov }}" class="absolute inset-0 w-full h-full object-cover z-0" />
@else
        ...
        <div class="flex flex-col md:flex-row items-start md:items-end justify-between gap-8 w-full">
            ...
            <div class="relative z-30 max-w-xl w-full">
                ...
                <div class="flex flex-wrap gap-4">
                    @if($dugme_link)
                        <a href="{{ $dugme_link['url'] }}" class="bg-[#4F7052] hover:bg-[#3d5a40] text-white px-8 py-4 rounded-[20px] font-bold transition duration-300">
                            {{ $dugme_tekst ?: 'Rezerviši' }}
                        </a>
                    @endif
